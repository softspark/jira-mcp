...
/** Validated config.json + normalized credentials + resolved global language. */
interface ConfigSources {
  readonly config: ReturnType<typeof ConfigFileSchema.parse>;
  readonly normalized: NormalizedCredentials;
  readonly globalLanguage: LanguageCode;
}

/**
 * Read, validate and normalize config.json + credentials.json.
 *
 * Both {@link loadConfig} (Jira projects) and {@link loadConfluenceConfig}
 * (Confluence spaces) build on this, so a single Atlassian site is described
 * once and both products read the same credentials.
 */
async function readSources(
  options?: LoadConfigOptions,
): Promise<ConfigSources> {
  ...
  return {
    config,
    normalized: normalizeCredentials(credsResult.data),
    // Resolve global language (config file → hardcoded default)
    globalLanguage: config.default_language ?? DEFAULT_LANGUAGE,
  };
}
...
export async function loadConfig(
  ...
  const { config, normalized, globalLanguage } = await readSources(options);
...
/**
 * Load and validate Confluence configuration with credentials.
 *
 * Reads the `spaces` section of config.json and merges in the same
 * credentials Jira uses, because one Atlassian Cloud site serves both
 * products from the same host under `/wiki`.
 *
 * An absent or empty `spaces` section is a configuration error rather than an
 * empty result: a Confluence server with no reachable space cannot serve a
 * single tool call, and failing at startup says so far more clearly than
 * every later call failing on its own.
 *
 * @throws {ConfigNotFoundError} If a required file is missing.
 * @throws {ConfigValidationError} If file content fails Zod validation or no
 *   space is configured.
 */
export async function loadConfluenceConfig(
  ...
  if (Object.keys(spaces).length === 0) {
    throw new ConfigValidationError(
      'No Confluence spaces configured. Add one with: confluence-mcp space add <SPACE_KEY> <https://your-site.atlassian.net>',
    );
  }

  const globalFormat: BodyFormat = config.default_format ?? DEFAULT_BODY_FORMAT;

  const mergedSpaces: Record<string, ConfluenceSpaceInstanceConfig> = {};
  for (const [key, space] of Object.entries(spaces)) {
    const cred = resolveCredentialForUrl(normalized, space.url);
    const instance = {
      url: space.url,
      username: cred.username,
      api_token: cred.api_token,
      language: space.language ?? globalLanguage,
      format: space.format ?? globalFormat,
    };

    const instanceResult =
      ConfluenceSpaceInstanceConfigSchema.safeParse(instance);
    if (!instanceResult.success) {
      throw new ConfigValidationError(
        `Space '${key}' validation failed: ${instanceResult.error.message}`,
      );
    }

    mergedSpaces[key] = instanceResult.data;
  }
...
/**
 * Get the space configuration for a specific Confluence space.
 *
 * Falls back to `default_space`, and then to the only configured space when
 * exactly one exists. A site with several spaces and no default forces the
 * caller to name one rather than silently picking whichever key happens to
 * be first in the JSON object.
 *
 * @returns Tuple of [resolvedSpaceKey, spaceConfig].
 * @throws {ConfigValidationError} If the space cannot be resolved.
 */
export function getSpaceConfig(
  config: ConfluenceConfig,
  spaceKey?: string,
): readonly [string, ConfluenceSpaceInstanceConfig] {
  const keys = Object.keys(config.spaces);
  const key =
    spaceKey ?? config.default_space ?? (keys.length === 1 ? keys[0] : undefined);

  if (key === undefined) {
    throw new ConfigValidationError(
      `No space_key given and no default_space configured. Configured spaces: ${keys.join(', ')}`,
    );
  }
  ...
  if (!space) {
    throw new ConfigValidationError(
      `Space '${key}' not found in configuration. Configured spaces: ${keys.join(', ')}`,
    );
  }

  return [key, space] as const;
}
