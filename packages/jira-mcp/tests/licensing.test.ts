...
describe('licensing', () => {
  ...
  it('the sibling published package carries the same licensing contract', () => {
    // confluence-mcp ships from the same repository under the same licence.
    // Asserting it here rather than duplicating this whole file keeps one
    // place to change when the contract does.
    const sibling = resolve(ROOT, '..', 'confluence-mcp');
    const readSibling = (rel: string): string =>
      readFileSync(join(sibling, rel), 'utf-8');

    expect(readSibling('LICENSE')).toContain('Apache License');
    expect(readSibling('NOTICE')).toContain('Lukasz Krzemien');

    const pkg = JSON.parse(readSibling('package.json')) as {
      license: string;
      files: string[];
    };
    expect(pkg.license).toBe('Apache-2.0');
    ...
    const config = readSibling('tsup.config.ts');
    expect(config).toContain('Apache-2.0');
    expect((config.match(/banner:/g) ?? []).length).toBe(2);
  });
});
