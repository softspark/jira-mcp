...
// Config loader and global paths both live in the shared core package, so one
// factory covers them. The rest of core is spread through untouched.
vi.mock('@softspark/atlassian-mcp-core', async (importOriginal) => {
  ...
  return {
    ...
    GLOBAL_TASK_TEMPLATE_DEFINITIONS_DIR:
      '/tmp/test-jira-mcp-config/templates/task-templates',
  };
});
