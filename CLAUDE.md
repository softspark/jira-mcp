...
- `kb/reference/`: architecture, api, adf, caching, configuration, templates, confluence, build-and-packaging.
- `kb/decisions/`: ADR-0001 hardening priorities, ADR-0002 one version across both published packages.
- `kb/howto/`: setup, multi-instance, cli-usage.
- `kb/procedures/`: sop-pre-commit, sop-release, sop-post-release-testing.
- `AGENTS.md`: shared ai-toolkit policy, auto-generated between the `TOOLKIT:` markers. Do not hand-edit.
- `rules/jira-mcp.md`, `rules/confluence-mcp.md`: agent rules for each server. **Hand-maintained**, and the source `ai-toolkit add-rule` publishes. `scripts/validate_counts.py` checks the Jira one names every tool, so update it when the tool surface changes.
