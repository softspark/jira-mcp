...
- **Read the `code` on a failure.** `INVALID_INPUT` and `TEMPLATE_MISSING_VAR` mean the call was wrong and nothing reached Jira: the message names what to pass, so fix the arguments and call again. Do not look for a way around the server. `UNKNOWN_ERROR` is the one that means something is actually broken.
